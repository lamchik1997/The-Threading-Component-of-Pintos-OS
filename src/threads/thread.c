...
/* Type Define function for list_insert order found in list.h.
   Compares the value of two list elements A and B, given
   auxiliary data AUX.  Returns true if A is less than B, or
   false if A is greater than or equal to B. In our case, AUX should
   be ignored. */
bool less_func (const struct list_elem *a,const struct list_elem *b, void * aux UNUSED){
...
thread_set_priority (int new_priority) 
{
    ...
    if(thread_current()->priority < old_P)
        thread_yield();
    //}
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  //list_insert_ordered (&all_list,&t->allelem,list_less,NULL);
  intr_set_level (old_level);
}
